-- Give every star holder a visible, server-authoritative half and only allow
-- opposite halves to form a star alliance. Existing live pairings are kept.

begin;

alter table symbol_pairing_assignments
  add column if not exists fragment_side text;
...
alter table symbol_pairing_assignments
  ...
  check (
    (symbol='STAR' and fragment_side in ('LEFT','RIGHT'))
    or (symbol='HEART' and fragment_side is null)
  ) not valid;
...
begin
  if new.symbol='HEART' then
    new.fragment_side:=null;
  elsif new.fragment_side is null then
    perform pg_advisory_xact_lock(hashtext('wedding-star-fragment-side-v1'));
    select count(*) filter(where fragment_side='LEFT'),count(*) filter(where fragment_side='RIGHT')
      ...
      where symbol='STAR' and guest_id<>new.guest_id;
    new.fragment_side:=case when v_left<=v_right then 'LEFT' else 'RIGHT' end;
  end if;
  return new;
end; $$;

drop trigger if exists symbol_pairing_assign_fragment_side on symbol_pairing_assignments;
create trigger symbol_pairing_assign_fragment_side
before insert or update of symbol,fragment_side on symbol_pairing_assignments
for each row execute function assign_star_fragment_side();

-- Preserve every existing active or pending relationship and make its members
-- complementary before balancing the remaining live star holders.
...
where r.relationship_type='STAR_ALLIANCE' and r.status in ('PENDING','ACTIVE')
  and s.symbol='STAR' and s.guest_id in(r.player_a_id,r.player_b_id);

update symbol_pairing_assignments
set fragment_side=null
where symbol='STAR' and fragment_side is null;
...
begin
  ...
  if p_relationship_type in ('CUPID_ALLIANCE','STAR_ALLIANCE') then
    ...
    if p_relationship_type='STAR_ALLIANCE' then
      select fragment_side into v_guest_fragment from symbol_pairing_assignments where guest_id=v_guest.id and symbol='STAR';
      select fragment_side into v_target_fragment from symbol_pairing_assignments where guest_id=v_target.id and symbol='STAR';
      if v_guest_fragment is null or v_target_fragment is null or v_guest_fragment=v_target_fragment then
        raise exception using errcode='P0001',message='star_fragment_side_mismatch';
      end if;
    end if;
...
insert into audit_log(actor,action,target_type,target_id,details)
values('migration:202607310009','star.fragment_pairing_enabled','game_state','1',jsonb_build_object(
  'existing_relationships_preserved',true,'opposite_halves_required',true));
