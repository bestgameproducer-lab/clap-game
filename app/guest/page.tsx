...
export default function GuestPage() {
  ...
  const [contentNotice, setContentNotice] = useState<{ title: string; detail: string; signature: string } | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        let nextNotice: { title: string; detail: string; signature: string } | null = null;
        if (previousSnapshot && previousSnapshot.guestId === nextSnapshot.guestId) {
          ...
          try {
            ...
            if (saved?.guestKey === guestKey && saved.signature && saved.signature !== activitySignature) {
  ...
  function renderSymbolPairing(assignment: GuestData['assignments'][number]) {
    ...
    return <section className={`inline-symbol-pairing ${isStarTask ? 'star' : 'heart'}`} aria-label={isStarTask ? '星星伙伴配对' : '爱心伙伴配对'}>
      ...
      {pairing.status === 'UNPAIRED_FINAL' ? <div className="story-unlock lonely"><strong>{isStarTask ? '领航星' : '孤单丘比特'}</strong><p>{isStarTask ? '你不属于某一个固定组合，而是为所有人指引方向。任务已完成。' : '你就是帮助别人相遇的丘比特。任务已完成，并获得与成功配对者相同的积分。'}</p></div> : isPaired ? <div className="story-unlock"><strong>{isStarTask ? '星光联盟' : '丘比特联盟'}已成立</strong><p>你与 {relationship.partnerName} 已完成双向确认。</p></div> : <div className="connection-form"><label htmlFor={inputId}>对方的玩家编号</label><div><input id={inputId} value={connectionTargetCode} onChange={(event) => setConnectionTargetCode(normalizePlayerCode(event.target.value))} maxLength={5} placeholder="例如 K7M4" autoCapitalize="characters" autoCorrect="off" spellCheck={false}/><button disabled={busy || offline || !phaseOneInteractionsOpen || !isPlayerCode(connectionTargetCode)} onClick={() => void connectPlayer(relationshipType)}>{isStarTask ? '邀请另一半星星' : '邀请爱心伙伴'}</button></div>{relationship?.status === 'PENDING' && <div className="pending-connection"><p>{relationship.confirmedByMe ? `已提交，等待 ${relationship.partnerName} 输入你的编号。` : `${relationship.partnerName} 邀请你配对；输入对方编号即可接受。`}</p><button type="button" className="text-button" disabled={busy || offline} onClick={() => void rejectConnection(relationship.id)}>拒绝这项邀请</button></div>}</div>}
    </section>;
  }
  ...
  function renderPhaseTwoAction(assignment: GuestData['assignments'][number]) {
    ...
    return <div className="phase-two-action"><strong>{isHeart ? '爱与恨的秘密选择' : '星光秘密选择'}</strong><div className="phase-two-secret-warning"><strong>必须秘密选择 · 不能商量</strong><p>请独立作出决定，不要询问、暗示或约定彼此的选择。</p></div><section className="phase-two-payoff" aria-label={`${isHeart ? '爱与恨' : '星光'}抉择积分规则`}><strong>积分规则 · 先看清再选择</strong><div><span>双方都选「{cooperative}」</span><b>各得 3 分</b></div><div><span>你选「{cooperative}」，伙伴选「{selfish}」</span><b>你 0 分 · 伙伴 5 分</b></div><div><span>你选「{selfish}」，伙伴选「{cooperative}」</span><b>你 5 分 · 伙伴 0 分</b></div><div><span>双方都选「{selfish}」</span><b>各得 1 分</b></div><p>选择合作更稳；选择「{selfish}」可能拿到 5 分，但双方都这样选时只能各得 1 分。</p></section><div className="phase-two-choice-buttons">{choices.map((choice) => <button type="button" className={phaseTwoDilemmaChoice === choice.value ? 'selected' : ''} disabled={busy || offline || !actionOpen} key={choice.value} onClick={() => setPhaseTwoDilemmaChoice(choice.value)}>{choice.label}</button>)}</div><button disabled={busy || offline || !actionOpen || !phaseTwoDilemmaChoice} onClick={() => void submitPhaseTwoAction({ action: 'dilemma', choice: phaseTwoDilemmaChoice }, PENDING_DILEMMA_MESSAGE)}>确认提交 · 不可修改</button></div>;
  }
  ...
  return <main className="dashboard-shell">
    ...
    {isActivePlayer && data.game?.results_visible && data.results && <section className="reveal-card"><small>THE FINAL REVEAL</small><h2>身份揭晓</h2>{data.results.votedTargetName ? <div className={`vote-verdict ${data.results.voteCorrect ? 'correct' : 'missed'}`}><span>你投给了 {data.results.votedTargetName}</span><strong>{data.results.voteCorrect ? `成功找到恶作剧者 · 获得 ${data.results.bonusPoints} 分` : '恶作剧者成功隐藏了自己'}</strong></div> : <div className="vote-verdict missed"><strong>你没有提交最终投票</strong></div>}<div className="team-role-reveal">{data.results.teamMembers.map((member) => <div key={member.id}><span>{member.name}</span><strong>{member.is_hidden_spy ? '丘比特的暗线恶作剧者' : ROLE_LABELS[member.role]?.title ?? member.role}</strong></div>)}</div><p>感谢你成为这场婚礼故事的一部分。</p></section>}
    ...
    {secretReaderOpen && !hasPublicIdentity && !usesTricksterFacade && <div className="secret-reader-backdrop">
      <section className={`secret-reader-dialog ${isTrickster ? 'trickster' : ''}`} role="dialog" aria-modal="true" aria-labelledby="secret-reader-title">
        ...
        <div className="secret-reader-content">
          ...
          {usesTricksterFacade && missionStory && <section className="story-connection-card trickster secret-reader-command"><div className="section-heading"><div><small>CUPID'S CALL</small><h2>丘比特的召集令</h2></div><span>{missionStory.tricksterAttemptsUsed}/{missionStory.tricksterMaxAttempts}</span></div><div className="signal-script"><small>暗号问句</small><strong>你觉得丘比特今天心情怎么样？</strong><small>正确回答</small><strong>他好像想开个玩笑。</strong></div>{!canUseTricksterSignal ? <p className="trickster-waiting-note">{data.game?.stage === 'task_round_1' ? '婚礼仪式进行中，秘密确认暂时暂停；仪式结束后会重新开放。' : '秘密确认入口当前已经关闭。'}</p> : tricksterRelationship?.status === 'ACTIVE' ? <div className="story-unlock"><strong>已找到同伴</strong><p>你和 {tricksterRelationship.partnerName} 已完成双向确认。继续隐藏身份。</p></div> : <div className="connection-form"><label htmlFor="trickster-partner-code">暗号匹配后，输入对方玩家编号</label><div><input id="trickster-partner-code" value={connectionTargetCode} onChange={(event) => setConnectionTargetCode(normalizePlayerCode(event.target.value))} maxLength={5} placeholder="例如 K7M4" autoCapitalize="characters" autoCorrect="off" spellCheck={false}/><button disabled={busy || offline || missionStory.tricksterAttemptsUsed >= missionStory.tricksterMaxAttempts || !isPlayerCode(connectionTargetCode)} onClick={() => void connectPlayer('TRICKSTER_CONNECTION')}>秘密确认</button></div>{tricksterRelationship?.status === 'PENDING' && <p>{tricksterRelationship.confirmedByMe ? `已提交，等待 ${tricksterRelationship.partnerName} 输入你的编号。` : `${tricksterRelationship.partnerName} 已通过暗号找到你，请输入对方编号。`}</p>}<p>本阶段最多试探 {missionStory.tricksterMaxAttempts} 位宾客。不要连续询问，也不要直接暴露身份。</p></div>}</section>}
        </div>
