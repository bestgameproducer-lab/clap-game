...
test('social missions support bounded mutual confirmation with atomic completion', async () => {
  const [migration, route, guestData, guestPage, guestTaskUi, styles] = await Promise.all([
    ...
    readFile(new URL('../lib/guest-task-ui.ts', import.meta.url), 'utf8'),
  ...
  assert.match(guestPage, /guestPhotoEvidenceLabel\(assignment\.task\.mission_code/);
  assert.match(guestTaskUi, /'P1-SOCIAL-002': '添加新郎新娘同框照片'/);
  assert.match(guestPage, /拍摄照片或从相册选择/);
  assert.match(guestTaskUi, /'P1-SOCIAL-001': '添加与新朋友的合影'/);
...
test('task reassignment cancels the prior task while retaining linked audit history', async () => {
  ...
  assert.match(adminData, /rpc\('reassign_task_assignment_for_run'/);
  assert.match(adminPage, /演示任务派发/);
  assert.match(adminPage, /改派会保留旧任务的审计记录并将其标记为已取消/);
