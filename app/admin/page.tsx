...
type Task = { id: string; title: string; description: string; points: number; role_scope: string; category: string; stage: string; active: boolean; grants_hidden_spy: boolean };
...
export default function AdminPage() {
  ...
  const [newTask, setNewTask] = useState({ title: '', description: '', points: '20', roleScope: 'all', category: 'standard', stage: 'task_round_1', active: true, grantsHiddenSpy: false });
  ...
  useEffect(() => {
    if (libraryTaskId === 'new') { setNewTask({ title: '', description: '', points: '20', roleScope: 'all', category: 'standard', stage: 'task_round_1', active: true, grantsHiddenSpy: false }); return; }
    const task = data?.tasks.find((item) => item.id === libraryTaskId);
    if (task) setNewTask({ title: task.title, description: task.description, points: String(task.points), roleScope: task.role_scope, category: task.category, stage: task.stage, active: task.active, grantsHiddenSpy: task.grants_hidden_spy });
  }, [data, libraryTaskId]);
