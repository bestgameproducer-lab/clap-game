...
export default function AdminPage() {
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-grid">
      ...
      <article className="section-card"><div className="section-heading"><div><small>HELPER RECORDS</small><h2>丘比特帮手记录</h2></div><span>{data.helperActions.length}</span></div><p className="muted">这些是帮手提交的秘密保护行动，仅供管理员核验。第一阶段不进行最终计分。</p>{data.helperActions.length === 0 ? <div className="empty-state">尚无帮助记录。</div> : <div className="relationship-admin-list">{data.helperActions.map((item) => <div key={item.id}><strong>{item.helper?.name} → {item.trickster?.name}</strong><span>{item.note}</span><small>{item.status === 'CONFIRMED' ? '已核验' : '待后续核验'}</small></div>)}</div>}<div className="control-state">{data.game?.phase_one_completed_at ? '第一阶段已经统一结算' : '切换到第二阶段时，系统将先检查两组图案是否均形成两对联盟，再自动产生孤单丘比特和领航星。'}</div></article>
    </section>
