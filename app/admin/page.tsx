  ...
  return <main className="admin-shell">
    ...
    <section className="section-card finale-workflow-card">
      ...
      <div className="finale-workflow-steps">
        ...
        <article className={finalResultsLocked ? 'done' : ''}><span className="finale-step-index">04</span><div><strong>公布身份并结算终局个人奖励</strong><small>{finalResultsLocked ? `投票命中奖励已结算 +${settledPersonalPoints} 分 · 本场数据已永久冻结` : data.game?.voting_open ? '请先关闭本轮投票，再公布身份并结算' : data.votes.length === 0 ? '至少收到 1 份本轮投票后才能进行不可撤回的公布' : `已投 ${data.votes.length} / 应投 ${competitiveDrawn} / 缺席 ${missingFinalVotes}；只结算个人奖励，团队挑战分不会变化`}</small></div><button type="button" className={finalResultsLocked ? 'secondary' : ''} disabled={busy || finalResultsLocked || Boolean(data.game?.voting_open) || (data.game?.voting_round ?? 0) < 1 || data.votes.length === 0} onClick={requestResultsToggle}>{finalResultsLocked ? '终局已永久发布' : data.game?.voting_open ? '请先关闭投票' : data.votes.length === 0 ? '等待本轮投票' : '公布身份并结算个人奖励'}</button></article>
      ...
      {pendingResultsVisible !== null && <section className="finale-confirmation" role="dialog" aria-label="确认公布身份"><div><small>请确认不可撤回的终局操作</small><strong>公布身份并结算终局个人奖励</strong><p>本轮已投 {data.votes.length} 人 / 应投 {competitiveDrawn} 人 / 缺席 {missingFinalVotes} 人。继续后将结算终局奖励，并永久冻结任务审核、积分、线索、奖项和任务配置。公开结果不能撤回。</p></div><div><button type="button" onClick={() => void confirmResultsToggle()} disabled={busy || Boolean(data.game?.voting_open)}>确认公布并永久冻结</button><button type="button" className="secondary" onClick={() => setPendingResultsVisible(null)} disabled={busy}>取消</button></div></section>}
