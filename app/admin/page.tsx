...
type AdminData = {
  ...
  helperActions: Array<{ id: string; note: string; status: string; created_at: string; helper?: { id: string; name: string }; trickster?: { id: string; name: string } }>;
  ...
  game: { registration_open: boolean; stage: string; voting_open: boolean; voting_round: number; results_visible: boolean; scoreboard_visible: boolean; phase_note: string | null; display_title: string | null; display_body: string | null; public_clue: string | null; timer_ends_at: string | null; invitation_code_updated_at: string | null; task_catalog_mode: 'demo' | 'live'; trickster_max_attempts: number; phase_one_completed_at: string | null } | null;
};
...
export default function AdminPage() {
  ...
  const [hiddenRole, setHiddenRole] = useState('NONE');
  ...
  const selectedGuestProfileSignature = selectedGuest ? `${selectedGuest.id}|${selectedGuest.team}|${selectedGuest.role}|${selectedGuest.story_role}|${selectedGuest.hidden_role}` : '';
  ...
  useEffect(() => {
    ...
    setHiddenRole(selectedGuest.hidden_role);
  ...
  return <main className="admin-shell">
    ...
    <details className="admin-advanced-tools"><summary>高级操作：预设身份、派发任务、线索与人工积分</summary><section className="section-card"><div className="section-heading"><div><small>QUICK OPERATIONS</small><h2>宾客操作台</h2></div></div>
      ...
      {selectedGuest && <div className="operation-grid">
        ...
        <form onSubmit={(event) => { event.preventDefault(); void action({ type: 'configureHiddenRole', guestId: selectedGuest.id, hiddenRole }, '隐藏特殊身份已保存'); }}><h3>丘比特的帮手</h3><p className="muted">全场最多一人。帮手不能同时担任仪式、图案角色或恶作剧者。</p><label htmlFor="guest-hidden-role">隐藏特殊身份</label><select id="guest-hidden-role" value={hiddenRole} onChange={(event) => setHiddenRole(event.target.value)}><option value="NONE">无</option><option value="CUPID_HELPER">丘比特的帮手</option></select><button disabled={busy || Boolean(selectedGuest.drawn_at) || selectedGuest.participation_mode !== 'ACTIVE_PLAYER'}>保存隐藏身份</button></form>
    ...
    <section className="admin-grid">
      ...
      <article className="section-card"><div className="section-heading"><div><small>HELPER RECORDS</small><h2>丘比特帮手记录</h2></div><span>{data.helperActions.length}</span></div><p className="muted">这些是帮手提交的秘密保护行动，仅供管理员核验。第一阶段不进行最终计分。</p>{data.helperActions.length === 0 ? <div className="empty-state">尚无帮助记录。</div> : <div className="relationship-admin-list">{data.helperActions.map((item) => <div key={item.id}><strong>{item.helper?.name} → {item.trickster?.name}</strong><span>{item.note}</span><small>{item.status === 'CONFIRMED' ? '已核验' : '待后续核验'}</small></div>)}</div>}<div className="control-state">{data.game?.phase_one_completed_at ? '第一阶段已经统一结算' : '仪式结束后仍可继续提交第一阶段任务；开启最终投票时，系统才会检查两组图案配对并统一关闭未完成任务。'}</div></article>
    </section>
