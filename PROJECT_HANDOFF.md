...
- `/admin`：主办方控制台
- `/host`：主持人流程台、完整婚礼流程控制、团队/个人加分与终局投票结算
- `/station`：现场任务核验站
- `/scoreboard`：公开投影大屏
- `/admin/cards`：可打印宾客卡片
...
- 生产已应用 `202607310003` 至 `202607310009`；`202607310010_fix_admin_password_pgcrypto_path.sql` 修复托管 Supabase 中管理员密码校验与轮换函数的 pgcrypto 搜索路径，发布后应成为最新迁移。星星角色面向宾客显示为“星光寻觅者”；爱心与星光身份均为秘密身份，只有仪式职务公开。
...
- 宾客可在仪式前（`registration`、`waiting`）和仪式结束后（`ceremony_end`、`task_round_2`、`group_game`）提交第一阶段任务；婚礼仪式进行中（`task_round_1`）暂停提交，进入投票或揭晓后关闭。
...
代码中的八个环节及宾客默认提示在 `lib/game-stages.ts`：

1. `registration`：婚礼入场 · 宾客签到
2. `waiting`：婚礼序章 · 等待仪式
3. `task_round_1`：婚礼仪式 · 丘比特的秘密来宾
4. `ceremony_end`：仪式结束 · 第一阶段恢复
5. `task_round_2`：婚宴前奏 · 第二阶段开启
6. `group_game`：婚宴互动 · 团队挑战
7. `voting`：婚礼终章 · 最终投票
8. `results`：婚礼终章 · 身份揭晓

管理员名称、宾客看到的名称和实际婚礼阶段应一致。`ceremony_end` 只恢复第一阶段提交，不派发第二阶段任务；进入 `task_round_2` 才会原子结束第一阶段并统一派发第二阶段。阶段切换必须通过数据库状态机；投票和揭晓使用专用操作，不能靠下拉框绕过结算。

### 后台与现场页面

- 管理台使用短入口/标签分区，不要把所有工具堆成一张超长页面。
- 待审核任务按最新提交在最上方，每项可折叠。
- 主持人页面开放私密全员总览、完整婚礼流程控制、团队加分、个人加分和终局投票结算；总览显示分组、当前分数和恶作剧者。主持人可通过二次确认切换六个常规婚礼阶段，也可发起/关闭最终投票，并在二次确认后公布身份和结算；主持人只看到投票人数，不看到个人选票。所有操作均鉴权并写审计日志。
