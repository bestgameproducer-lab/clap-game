...
type AdminData = {
  guests: Array<{ id:string; name:string; login_name:string; team:string; points:number; claimed_at:string|null }>;
...
export default function AdminPage() {
  ...
  return <main className="admin-shell">
    ...
    <section className="section-card"><div className="section-heading"><div><small>GUESTS</small><h2>宾客进度</h2></div><span>{data.guests.length}</span></div><div className="guest-admin-list">{data.guests.map((guest)=><article key={guest.id}><div className="guest-avatar">{guest.name.slice(0,1)}</div><div><strong>{guest.name}</strong><small>{guest.login_name} · {guest.team} · {guest.points} 分</small></div><span className={guest.claimed_at?'claimed':'unclaimed'}>{guest.claimed_at?'已认领':'未认领'}</span>{guest.claimed_at&&<button className="mini-button" disabled={busy} onClick={()=>action({type:'resetGuestClaim',guestId:guest.id})}>重置</button>}</article>)}</div></section>
