...
declare
  ...
  v_fixed_role_branch constant text := $q$if v_guest.team='家人组' or lower(v_guest.login_name)='yirui zhang' or v_guest.story_role<>'NONE' or not v_guest.eligible_for_secret_role or (v_guest.role_locked and v_guest.role='guest') then
    v_role:='guest';
...
begin
  ...
  if position('v_reserved_guests integer;' in v_draw_definition)=0
    or position('select role_name into v_role from(' in v_draw_definition)=0
  ...
  v_draw_definition:=regexp_replace(
    v_draw_definition,
    $pattern$if v_guest\.team='家人组' or lower\(v_guest\.login_name\)='yirui zhang' or v_guest\.story_role<>'NONE' or not v_guest\.eligible_for_secret_role or v_guest\.role_locked then\s+v_role:='guest';\s+else$pattern$,
    v_fixed_role_branch
  );
  v_draw_definition:=replace(
    v_draw_definition,
    'select role_name into v_role from(',
    $q$select count(*) into v_reserved_spies from guests g where g.id<>v_guest.id and g.active and g.phase_two_eligible
      and g.drawn_at is null and g.team=v_guest.team and g.role_locked and g.role='spy';
    select role_name into v_role from($q$
  );
  v_draw_definition:=replace(
    v_draw_definition,
  ...
  if position('(v_guest.role_locked and v_guest.role=''guest'')' in v_draw_definition)=0
    or position('v_role:=v_guest.role' in v_draw_definition)=0
    or position('v_reserved_spies from guests' in v_draw_definition)=0
    or position('1-v_drawn_spies-v_reserved_spies' in v_draw_definition)=0 then
    raise exception using errcode='P0001',message='latest_draw_preset_patch_verification_failed';
  end if;
