...
export default function GuestPage() {
  ...
  const tricksterFacadeAssignment = usesTricksterFacade ? dashboardAssignments.find((assignment) => assignment.is_initial) : undefined;
  const openTricksterDossier = () => {
    if (!tricksterFacadeAssignment) return;
    setShowSecrets(false);
    setSecretReaderOpen(false);
    setExpandedAssignments((current) => ({ ...current, [tricksterFacadeAssignment.id]: true }));
    window.setTimeout(() => document.getElementById(`mission-${tricksterFacadeAssignment.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 60);
  };
  return <main className="dashboard-shell">
      ...
      <div className={`identity-strip ${identityVisible ? 'visible' : 'concealed'} ${isTrickster && identityVisible && !data.game?.results_visible ? 'trickster-identity' : ''}`}>
        <div className="identity-strip-heading">
          ...
          {!hasPublicIdentity && <div className="identity-private-actions">
            ...
            <button type="button" className="identity-reader-button" onClick={() => usesTricksterFacade ? openTricksterDossier() : (setShowSecrets(false), setSecretReaderOpen(true))}>展开阅读</button>
          </div>}
        </div>
    ...
    {isActivePlayer && <section className="section-card"><div className="section-heading"><div><small>SECRET MISSIONS</small><h2>我的秘密任务</h2></div><span>{dashboardAssignments.length}</span></div>
      ...
      {dashboardAssignments.length === 0 ? <div className="empty-state">抽卡后，你领取的第一项任务会立即显示在这里。</div> : dashboardAssignments.map((assignment, index) => {
        const opensTricksterDossier = usesTricksterFacade && assignment.id === tricksterFacadeAssignment?.id;
        return <details id={`mission-${assignment.id}`} className={`mission-item ${opensTricksterDossier ? 'trickster-facade' : ''}`} key={assignment.id} open={expandedAssignments[assignment.id] ?? false} onToggle={(event) => { const open = event.currentTarget.open; setExpandedAssignments((current) => current[assignment.id] === open ? current : { ...current, [assignment.id]: open }); }}><summary className="mission-summary"><span className="mission-number">{String(index + 1).padStart(2, '0')}</span><span className="mission-summary-copy"><span className="mission-meta"><span>{assignment.task.points} 分</span><span className={`status ${assignment.status}`}>{STATUS_LABELS[assignment.status] ?? assignment.status}</span></span><strong>{assignment.task.title}</strong></span><span className="mission-chevron" aria-hidden="true">⌄</span></summary><div className="mission-body">
          {opensTricksterDossier && <section className="trickster-dossier-inline"><div className="trickster-dossier-intro"><small>PRIVATE · EYES ONLY</small><h3>你是丘比特的恶作剧者</h3><p>这张普通任务是你的公开伪装。真正的身份与密令只在这里出现；阅读后再次点击上方任务卡即可关闭。</p></div><div className="secret-reader-rule critical"><strong>必须隐藏身份</strong><p>不要承认身份、不要展示本页、不要直接询问别人是不是同伴；继续像普通宾客一样行动。</p></div><div className="trickster-true-missions"><small>真正的间谍任务</small>{trueTricksterAssignments.length === 0 ? <p>密令正在准备中，请稍后刷新。</p> : trueTricksterAssignments.map((secretAssignment) => <article key={secretAssignment.id}><strong>{secretAssignment.task.title}</strong><p>{secretAssignment.task.description}</p><span>{secretAssignment.task.verification_method}</span></article>)}</div>{missionStory && <section className="story-connection-card trickster trickster-command-inline"><div className="section-heading"><div><small>CUPID'S CALL</small><h2>丘比特的召集令</h2></div><span>{missionStory.tricksterAttemptsUsed}/{missionStory.tricksterMaxAttempts}</span></div><div className="signal-script"><small>暗号问句</small><strong>你觉得丘比特今天心情怎么样？</strong><small>正确回答</small><strong>他好像想开个玩笑。</strong></div>{!canUseTricksterSignal ? <p className="trickster-waiting-note">先记住暗号。主持人开启第一轮后，秘密确认入口会在这里开放。</p> : tricksterRelationship?.status === 'ACTIVE' ? <div className="story-unlock"><strong>已找到同伴</strong><p>你和 {tricksterRelationship.partnerName} 已完成双向确认。继续隐藏身份。</p></div> : <div className="connection-form"><label htmlFor="trickster-partner-code">暗号匹配后，输入对方玩家编号</label><div><input id="trickster-partner-code" value={connectionTargetCode} onChange={(event) => setConnectionTargetCode(event.target.value.toUpperCase())} maxLength={7} placeholder="例如 P012"/><button disabled={busy || offline || missionStory.tricksterAttemptsUsed >= missionStory.tricksterMaxAttempts || !/^P[0-9]{3,6}$/.test(connectionTargetCode)} onClick={() => void connectPlayer('TRICKSTER_CONNECTION')}>秘密确认</button></div>{tricksterRelationship?.status === 'PENDING' && <p>{tricksterRelationship.confirmedByMe ? `已提交，等待 ${tricksterRelationship.partnerName} 输入你的编号。` : `${tricksterRelationship.partnerName} 已通过暗号找到你，请输入对方编号。`}</p>}<p>本阶段最多试探 {missionStory.tricksterMaxAttempts} 位宾客。不要连续询问，也不要直接暴露身份。</p></div>}</section>}</section>}
          <div className={opensTricksterDossier ? 'decoy-task-details' : undefined}>{opensTricksterDossier && <small>你的公开伪装任务</small>}<p>{assignment.task.description}</p>{!isTaskActionOpenAtStage(assignment.task.stage, data.game?.stage) && (assignment.status === 'assigned' || assignment.status === 'rejected') && <div className="task-feedback">{isTaskWaitingForStage(assignment.task.stage, data.game?.stage) ? '任务已领取。请先记住内容，等待主持人宣布本轮开始后再执行并提交。' : '本环节已停止提交；如需补录，请到任务站联系工作人员。'}</div>}<div className="verification-note"><strong>如何验证</strong><span>{assignment.task.verification_method}</span></div>{assignment.evidence_url && <figure className="evidence-preview"><a href={assignment.evidence_url} target="_blank" rel="noreferrer"><img src={assignment.evidence_url} alt={`${assignment.task.title}的验证照片`} loading="lazy"/></a><figcaption>验证照片 · 仅你和工作人员可见</figcaption></figure>}{isTaskActionOpenAtStage(assignment.task.stage, data.game?.stage) && (assignment.status === 'assigned' || assignment.status === 'rejected') && assignment.task.mechanic === 'STANDARD' && <div className="evidence-controls"><label htmlFor={`evidence-${assignment.id}`}>{assignment.evidence_url ? '更换验证照片' : '添加验证照片（选填）'}</label><input id={`evidence-${assignment.id}`} type="file" accept="image/*" disabled={offline || evidenceBusyId === assignment.id} onChange={(event) => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ''; if (file) void uploadEvidence(assignment.id, file); }}/>{assignment.evidence_url && <button type="button" className="text-button" disabled={offline || evidenceBusyId === assignment.id} onClick={() => { if (window.confirm('删除这张验证照片？')) void removeEvidence(assignment.id); }}>删除照片</button>}{evidenceBusyId === assignment.id && <small>正在压缩并安全上传…</small>}</div>}{assignment.completion_note && <div className="submission-note"><strong>我的完成说明</strong><span>{assignment.completion_note}</span></div>}{assignment.status === 'approved' && assignment.verification_note && <div className="submission-note approved"><strong>任务站核验记录</strong><span>{assignment.verification_note}</span></div>}{assignment.status === 'rejected' && <div className="task-feedback">任务站留言：{assignment.rejection_reason || '请补充验证后再次提交。'}</div>}{isTaskActionOpenAtStage(assignment.task.stage, data.game?.stage) && (assignment.status === 'assigned' || assignment.status === 'rejected') && !['HEART_MATCH','STAR_MATCH','TRICKSTER_SIGNAL','INSTANT_BONUS'].includes(assignment.task.mechanic) && <div className="submission-form"><label htmlFor={`completion-note-${assignment.id}`}>完成说明（选填）</label><textarea id={`completion-note-${assignment.id}`} value={completionNotes[assignment.id] ?? assignment.completion_note ?? ''} onChange={(event) => setCompletionNotes({ ...completionNotes, [assignment.id]: event.target.value })} maxLength={500} placeholder="例如：已完成合影，照片会在任务站出示。"/><button disabled={busy || offline || evidenceBusyId === assignment.id} onClick={() => submit(assignment.id, completionNotes[assignment.id] ?? assignment.completion_note ?? '')}>{offline ? '联网后可提交' : assignment.status === 'rejected' ? '补充完成 · 再次提交' : '我已完成 · 提交验证'}</button></div>}</div>
        </div></details>;
      })}
    </section>}
