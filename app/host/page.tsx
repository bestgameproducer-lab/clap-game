...
export default function HostPage() {
  ...
  return <main className="host-shell host-score-shell">
    ...
    {mode === 'overview' ? <section className="section-card host-score-panel"><div className="section-heading"><div><small>PRIVATE ROSTER</small><h2>分组、积分与身份</h2></div><span>{data.guests.length} 人</span></div><div className="host-roster-list">{data.guests.map((guest) => <article key={guest.id} className={(guest.role === 'spy' || guest.is_hidden_spy) && guest.drawn_at ? 'spy' : ''}><div><strong>{guest.name}</strong><small>{guest.team || '未分组'} · {guest.drawn_at ? '已抽卡' : guest.participation_mode === 'ACTIVE_PLAYER' ? '待抽卡' : '专属卡'}</small></div><span>{guest.eligible_for_personal_score ? `${guest.points} 分` : '不计分'}</span>{(guest.role === 'spy' || guest.is_hidden_spy) && guest.drawn_at && <b>{guest.is_hidden_spy ? '隐藏间谍' : '恶作剧者'}</b>}</article>)}</div></section> : mode === 'team' ? <section className="section-card host-score-panel"><div className="section-heading"><div><small>TEAM SCORE</small><h2>记录团队挑战成绩</h2></div></div>
      ...
      {data.game?.team_clues_settled_at && <div className="control-state on">团队积分已结算并锁定，不能继续计分。</div>}<form onSubmit={(event) => { event.preventDefault(); void addScore({ type: 'adjustTeamPoints', team: teamForm.team, amount: Number(teamForm.amount), reason: teamForm.reason }, `${teamForm.team} 已记录 ${teamForm.amount} 分`); }}>
        <label htmlFor="team-score-amount">本次记录分数</label><input id="team-score-amount" type="number" inputMode="numeric" min={0} max={100} value={teamForm.amount} onChange={(event) => setTeamForm({ ...teamForm, amount: event.target.value })} required/>
        ...
        <button disabled={busy || offline || Boolean(data.game?.team_clues_settled_at) || teamForm.amount === '' || Number(teamForm.amount) < 0 || Number(teamForm.amount) > 100 || !teamForm.reason.trim()}>{busy ? '保存中…' : `确认记录${teamForm.team} ${teamForm.amount || 0} 分`}</button>
      </form>
      ...
      <div className="host-stage-grid" aria-label="婚礼流程快捷切换">{HOST_STAGE_OPTIONS.map(([stage], index) => <button type="button" className={data.game?.stage === stage ? 'active' : ''} aria-current={data.game?.stage === stage ? 'step' : undefined} disabled={busy || offline || data.game?.stage === stage} key={stage} onClick={() => { setPendingFinaleAction(null); setPendingStage(stage); }}><small>{String(index + 1).padStart(2, '0')}</small><strong>{gameStageCopy(stage).title}</strong><em>{gameStageCopy(stage).roundLabel}</em></button>)}</div>
