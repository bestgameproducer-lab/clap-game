-- Persist hashed, per-client login failures so four-digit guest PINs cannot be guessed indefinitely.
create table if not exists guest_login_throttles (
  ...
  constraint guest_login_throttles_key_check check (attempt_key ~ '^[0-9a-f]{64}$'),
  constraint guest_login_throttles_failure_check check (failure_count between 0 and 5)
);

create index if not exists guest_login_throttles_guest_idx on guest_login_throttles (guest_id);
create index if not exists guest_login_throttles_updated_idx on guest_login_throttles (updated_at);
alter table guest_login_throttles enable row level security;
revoke all on table guest_login_throttles from public, anon, authenticated;
...
begin
  ...
  -- First-time PIN creation is not a password guess. It remains protected by the shared invitation code.
...
begin
  ...
  delete from guest_login_throttles where guest_id=p_guest_id;
