...
- 生产正式名单现为 32 位宾客、32 个登录账号；陈天然与陈子宥的旧联合家人账号及专属任务已由 forward-only migration 退役，批准记录、积分和审计历史继续保留。旧隐藏实体卡/隐藏第三名恶作剧者功能已退役；相关历史表、摘要与审计只为迁移兼容保留，不是当前现场配置。
...
- 正式 `P1-*`/`P2-*` 任务在后台只读，由版本化任务清单维护。自定义临时任务只允许在彩排模式创建、编辑和派发；正式婚礼模式始终锁定 21 项官方任务，现场不能用临时任务绕过清单。终局公布后，彩排模式中的自定义任务也一并冻结。
...
生产使用 32 位宾客、32 个登录账号的正式名单。完整基线位于：
...
7. `supabase/migrations/202608110001_add_joint_family_guest_account.sql` 与 `202608190001_retire_joint_family_guests.sql` — 联合家人账号的历史新增与当前安全退役
...
12. `lib/official-task-manifest.ts` — 正式 21 项任务的应用层白名单与任务元数据真相
...
当前生产状态：历史上已执行过彩排清场，但不能据此推断当前运行为空；注册、投票和大屏状态以主控实时显示为准；正式名单为 32 位宾客、32 个登录账号，任务配置、初始预设、当前正式运行记录与审计均须保留。请分别只读核对最新 `main`、Vercel release SHA 和 Supabase migration history；若待执行迁移尚未完整部署，必须按数据库上线文档在单次持续维护窗口内分批执行。
