...
test('content library mutations are validated, audited, and server-authoritative', async () => {
  ...
  assert.match(route, /requiredEnum\(body\.teamScope, '线索适用队伍'/);
  ...
  const grouping = await readFile(new URL('../supabase/migrations/202607310032_explicit_team_clue_settlement.sql', import.meta.url), 'utf8');
  assert.match(grouping, /add column if not exists team_scope/);
  assert.match(grouping, /save_game_clue_v3/);
});
...
test('admin loads inactive library entries but only offers active content for assignment', async () => {
  ...
  assert.match(page, /task\.active && task\.story_role_scope === 'NONE'[\s\S]*?!\/\^P\[12\]-\/[\s\S]*?task_catalog_mode === 'demo'/);
  assert.match(page, /TEAMS\.map/);
  assert.match(page, /任务库管理/);
  assert.match(page, /团队线索库/);
});
