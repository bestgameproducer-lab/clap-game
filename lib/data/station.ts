...
export async function getStationData() {
  ...
  const [guests, assignments, tasks, clues, game] = await Promise.all([
    ...
    db.from('assignments').select('id,guest_id,status,is_initial,completion_rank,submitted_at,approved_at,rejected_at,rejection_reason,task:tasks(id,title,description,verification_method,points,category,stage)').order('created_at', { ascending: false }),
