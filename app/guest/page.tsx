...
// Keep the assignment workflow wired up while the couple finalizes the real task list.
// Switching this on must happen together with reviewing the final task catalogue.
const TASK_CONTENT_READY: boolean = false;
    ...
    return <main className="draw-shell"><section className="draw-stage">
      ...
      <div className={`secret-card-scene ${drawing ? 'drawing' : ''} ${revealedCard ? 'revealed' : ''}`}><div className="secret-card">
        ...
        <div className="secret-card-front"><small>你被选中成为</small><h2>{role?.title}</h2><p>{role?.note}</p>
          ...
          <div className="card-task">{TASK_CONTENT_READY ? <><span>第一项秘密任务 · {revealedCard?.task.points} 分</span><strong>{revealedCard?.task.title}</strong><p>{revealedCard?.task.description}</p></> : <><span>秘密任务</span><strong>任务内容待公布</strong><p>主办方正在整理最终任务清单，之后会统一更新。</p></>}</div>
        </div>
      </div></div>
