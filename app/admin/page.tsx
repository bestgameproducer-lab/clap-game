...
type AdminData = {
  ...
  rehearsalResetPreview: { claimed_guests: number; drawn_guests: number; assignments: number; evidence_files: number; avatar_files: number; votes: number; guest_clues: number; clue_library_entries: number; personal_ledger_entries: number; team_ledger_entries: number; spy_ledger_entries: number; resource_ledger_entries: number; registration_open: boolean; voting_open: boolean; scoreboard_visible: boolean };
...
export default function AdminPage() {
  ...
  const rehearsalDataCount = resetPreview.claimed_guests + resetPreview.drawn_guests + resetPreview.assignments + resetPreview.votes
    + resetPreview.guest_clues + resetPreview.clue_library_entries + resetPreview.personal_ledger_entries + resetPreview.team_ledger_entries
  ...
  return <main className="admin-shell">
    ...
    <section className="section-card danger-zone"><div className="section-heading"><div><small>REHEARSAL RESET</small><h2>彩排数据安全清场</h2></div><span className={resetControlsClosed ? 'ready-badge' : 'warning-badge'}>{resetControlsClosed ? '公开入口已关闭' : '清场时将自动关闭公开入口'}</span></div><div className="reset-assurance"><strong>清场后，运行数据和线索库应全部归零</strong><p>系统会先自动关闭宾客注册、投票和公开大屏。保留宾客名单、锁定的队伍与初始身份、任务、历史主持内容、奖项名称和实体卡代码；清除所有宾客密码与登录、宾客自拍、抽卡结果、任务进度、验证照片、投票、个人与团队积分、历史竞拍流水、已发线索与整个线索库；同时清除配对、互认和第二轮临时状态。正式婚礼线索需要在清场后重新创建。</p></div><div className="reset-preview-grid"><div><strong>{resetPreview.claimed_guests}</strong><span>已认领宾客</span></div><div><strong>{resetPreview.assignments}</strong><span>任务记录</span></div><div><strong>{resetPreview.votes}</strong><span>投票记录</span></div><div><strong>{resetPreview.guest_clues}</strong><span>已发线索</span></div><div><strong>{resetPreview.clue_library_entries}</strong><span>线索库条目</span></div><div><strong>{resetPreview.avatar_files}</strong><span>宾客自拍</span></div><div><strong>{resetPreview.evidence_files}</strong><span>验证照片</span></div></div><form onSubmit={resetRehearsal}><label className="ready-check"><input type="checkbox" checked={resetForm.backupConfirmed} onChange={(event) => setResetForm({ ...resetForm, backupConfirmed: event.target.checked })}/><span><strong>我已下载上方七类 CSV 备份</strong><small>清场不可从网页撤销；审计日志会永久保留本次操作摘要。</small></span></label><label htmlFor="reset-reason">清场原因</label><input id="reset-reason" value={resetForm.reason} onChange={(event) => setResetForm({ ...resetForm, reason: event.target.value })} minLength={3} maxLength={300} required/><label htmlFor="reset-confirmation">输入 RESET WEDDING 确认</label><input id="reset-confirmation" value={resetForm.confirmation} onChange={(event) => setResetForm({ ...resetForm, confirmation: event.target.value })} autoComplete="off" spellCheck={false} placeholder="RESET WEDDING" required/><button className="danger" disabled={busy || !resetForm.backupConfirmed || resetForm.confirmation !== 'RESET WEDDING' || resetForm.reason.trim().length < 3}>{busy ? '正在安全清场…' : resetCleanupPending ? '重试照片清理' : '清空全部彩排运行数据'}</button></form></section></>}
    {pendingResetConfirmation && <section className="finale-confirmation" role="dialog" aria-modal="true" aria-label="最后确认彩排清场"><div><small>不可撤销的运行数据清理</small><strong>确认清空本次彩排？</strong><p>系统会先关闭注册、投票和公开大屏，再退出全部宾客并清除抽卡、任务进度、投票、积分和竞拍记录。34 位宾客、33 个登录账号与正式配置会保留。</p></div><div><button type="button" className="danger" disabled={busy} onClick={() => void confirmResetRehearsal()}>{busy ? '正在安全清场…' : '确认清空彩排数据'}</button><button type="button" className="secondary" disabled={busy} onClick={() => setPendingResetConfirmation(false)}>取消</button></div></section>}
  </main>;
}
