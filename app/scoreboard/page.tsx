...
export default function ScoreboardPage() {
  ...
  return <main className="scoreboard-shell">
    ...
    {data.resultsVisible && <section className="scoreboard-panel reveal-panel"><div className="scoreboard-title"><div><small>THE FINAL REVEAL</small><h2>身份揭晓</h2></div></div><div className="revealed-grid">{data.revealedRoles.map((guest) => <article key={guest.id}><small>{guest.team}</small><strong>{guest.name}</strong><span>{guest.is_hidden_spy ? '丘比特的暗线（隐藏间谍）' : guest.role === 'spy' ? '丘比特的恶作剧者' : '丘比特的秘密信使'}</span></article>)}</div>{data.voteCounts.length > 0 && <><h3>宾客投票</h3><div className="vote-result-list">{data.voteCounts.map((guest) => <div key={guest.id}><span>{guest.name} · {guest.team}</span><strong>{guest.votes} 票</strong></div>)}</div></>}</section>}
    ...
    <footer className="scoreboard-footer">每 10 秒自动更新 · 最近同步 {lastSyncLabel}{offlineReady ? ' · 离线刷新备用已准备' : ''}</footer>
  </main>;
}
