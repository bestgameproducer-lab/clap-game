...
export default function AdminPage() {
  ...
  const [newTask, setNewTask] = useState({ title: '', description: '', verificationMethod: DEFAULT_VERIFICATION_METHOD, points: '20', roleScope: 'all', category: 'standard', stage: 'task_round_1', active: true, grantsHiddenSpy: false });
  ...
  useEffect(() => {
    if (libraryTaskId === 'new') { setNewTask({ title: '', description: '', verificationMethod: DEFAULT_VERIFICATION_METHOD, points: '20', roleScope: 'all', category: 'standard', stage: 'task_round_1', active: true, grantsHiddenSpy: false }); return; }
  ...
  const readiness = [
    ...
    { label: '每位间谍已有专属线索', detail: spyGuests.length ? `${spiesWithActiveClues}/${spyGuests.length} 位已覆盖` : '请先预设间谍身份', ok: spyGuests.length > 0 && spiesWithActiveClues === spyGuests.length },
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-grid">
      <article className="section-card"><div className="section-heading"><div><small>TASK LIBRARY</small><h2>任务库管理</h2></div><span>{data.tasks.filter((task) => task.active).length}/{data.tasks.length} 启用</span></div><label htmlFor="library-task">选择任务或新建</label><select id="library-task" value={libraryTaskId} onChange={(event) => setLibraryTaskId(event.target.value)}><option value="new">＋ 新建任务</option>{data.tasks.map((task) => <option key={task.id} value={task.id}>{task.active ? '●' : '○'} {task.grants_hidden_spy ? '◆ ' : ''}{task.title}</option>)}</select><form onSubmit={(event) => { event.preventDefault(); void action({ type: 'saveTask', taskId: libraryTaskId === 'new' ? null : libraryTaskId, ...newTask, points: Number(newTask.points) }, libraryTaskId === 'new' ? '任务已加入任务库' : '任务已保存').then((ok) => { if (ok && libraryTaskId === 'new') setLibraryTaskId('new'); }); }}><label htmlFor="task-title">标题</label><input id="task-title" value={newTask.title} onChange={(event) => setNewTask({ ...newTask, title: event.target.value })} maxLength={120} required/><label htmlFor="task-description">任务说明</label><textarea id="task-description" value={newTask.description} onChange={(event) => setNewTask({ ...newTask, description: event.target.value })} maxLength={1000} required/><label htmlFor="task-verification">验证方式</label><textarea id="task-verification" value={newTask.verificationMethod} onChange={(event) => setNewTask({ ...newTask, verificationMethod: event.target.value })} maxLength={500} required/><p className="field-help">写清需要出示照片、提供口令、由相关宾客确认，或由工作人员现场观察。</p><div className="form-grid"><div><label htmlFor="task-points">积分</label><input id="task-points" type="number" min={1} max={500} value={newTask.points} onChange={(event) => setNewTask({ ...newTask, points: event.target.value })} required/></div><div><label htmlFor="task-role">适用身份</label><select id="task-role" value={newTask.roleScope} disabled={newTask.grantsHiddenSpy} onChange={(event) => setNewTask({ ...newTask, roleScope: event.target.value })}><option value="all">所有身份</option><option value="guest">祝福见证者</option><option value="spy">间谍</option><option value="helper">秘密信使</option></select></div><div><label htmlFor="task-category">类型</label><select id="task-category" value={newTask.category} disabled={newTask.grantsHiddenSpy} onChange={(event) => setNewTask({ ...newTask, category: event.target.value })}>{Object.entries(CATEGORY_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div><div><label htmlFor="task-stage">开放阶段</label><select id="task-stage" value={newTask.stage} disabled={newTask.grantsHiddenSpy} onChange={(event) => setNewTask({ ...newTask, stage: event.target.value })}>{STAGES.filter(([value]) => ['task_round_1', 'task_round_2', 'group_game'].includes(value)).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div></div><label className="ready-check"><input type="checkbox" checked={newTask.grantsHiddenSpy} onChange={(event) => setNewTask({ ...newTask, grantsHiddenSpy: event.target.checked, ...(event.target.checked ? { category: 'hidden', roleScope: 'guest', stage: 'task_round_2' } : {}) })}/><span><strong>完成后成为隐藏间谍</strong><small>全场只能启用并派发一张；审核通过时身份自动转化。</small></span></label><label className="ready-check"><input type="checkbox" checked={newTask.active} onChange={(event) => setNewTask({ ...newTask, active: event.target.checked })}/><span><strong>允许继续派发</strong><small>停用后不会影响已领取这项任务的宾客。</small></span></label>{libraryTaskId !== 'new' && <p className="field-help">任务一旦派发，积分、身份范围、类型、阶段和隐藏奖励会锁定；仍可修正文案或停用。</p>}<button disabled={busy}>{libraryTaskId === 'new' ? '创建任务' : '保存任务'}</button></form></article>
