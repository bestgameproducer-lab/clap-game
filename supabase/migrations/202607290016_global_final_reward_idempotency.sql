-- Final rewards belong to the wedding, not to a retry voting round.
drop index if exists result_rewards_guest_once_idx;
drop index if exists result_rewards_team_once_idx;
create unique index result_rewards_guest_once_idx
on result_rewards (reward_type, guest_id) where guest_id is not null;
create unique index result_rewards_team_once_idx
on result_rewards (reward_type, team) where team is not null;
...
begin
  ...
  for v_team in
    select voter.team, count(*)::integer as total_votes,
  ...
  for v_team in
    select g.team, count(*)::integer as total_guests,
