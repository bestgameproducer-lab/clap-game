-- Repair the live random draw without touching any completed draw. The prior
-- function reintroduced an ambiguous conflict target for trickster assignments
-- and only offered photo missions to uncast ordinary players, leaving the
-- random heart/star pools unreachable.
...
begin
  ...
  if v_guest.story_role<>'NONE' then
    ...
    -- Trickster facades do not consume the two scored guest slots per photo mission.
    ...
    -- The remaining fourteen ordinary players fill 5 heart, 5 star and four
    -- scored photo slots. Undrawn manual symbol presets reserve their slot.
    select * into v_task from tasks t where t.active and not t.is_demo and (
      (t.mission_code='P1-HEART-001' and
        (select count(*) from assignments a where a.task_id=t.id and a.is_initial)
        +(select count(*) from guests reserved where reserved.id<>v_guest.id and reserved.active
          and reserved.drawn_at is null and reserved.story_role='HEART_HOLDER')<5)
      or (t.mission_code='P1-STAR-001' and
        (select count(*) from assignments a where a.task_id=t.id and a.is_initial)
        +(select count(*) from guests reserved where reserved.id<>v_guest.id and reserved.active
          and reserved.drawn_at is null and reserved.story_role='STAR_HOLDER')<5)
  ...
  values('guest:'||v_guest.id::text,'guest.card_draw','guest',v_guest.id::text,jsonb_build_object(
    ...
    'mission_code',v_task.mission_code,'task_catalog_mode','phase-one-live-fixed'));
  return query select v_guest.team,v_guest.role,v_guest.story_role,v_guest.hidden_role,v_task.id,v_task.title,
...
insert into audit_log(actor,action,target_type,target_id,details)
values('migration:202607310008','phase_one.live_random_draw_fixed','game_state','1',jsonb_build_object(
  'trickster_conflict_target','assignments_guest_id_task_id_key','random_heart_slots',5,
  'random_star_slots',5,'scored_photo_slots',4,'existing_draws_preserved',true));
