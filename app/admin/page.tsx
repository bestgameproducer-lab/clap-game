...
const STORY_ROLE_LABELS: Record<string, string> = { NONE: '', OFFICIANT: '誓词引导人', RING_KEEPER: '戒指守护者', HEART_HOLDER: '爱心寻觅者', STAR_HOLDER: '星光寻觅者' };
const PHASE_TWO_MISSION_LABELS: Record<string, string> = {
  ...
  EXTRA_VOTE: '双重裁决 · 两票且投对奖励翻倍', SUPER_LUCKY: '超级幸运星 · 快照 + 2',
};
...
export default function AdminPage() {
  ...
  return <main className="admin-shell">
    ...
    <section className="section-card finale-workflow-card">
      ...
      {pendingResultsVisible !== null && <section className="finale-confirmation" role="dialog" aria-label="确认公布身份"><div><small>请确认不可撤回的终局操作</small><strong>公布身份并结算终局个人奖励</strong><p>本轮已投 {data.votes.length} 人 / 应投 {competitiveDrawn} 人 / 缺席 {missingFinalVotes} 人。系统逐队判断：抓住恶作剧者后，普通投中者 +2、双重裁决投中者 +4、其他已投票者 +1；逃脱队伍和未投票者 +0。继续后会永久冻结任务审核、积分、线索、奖项和任务配置，公开结果不能撤回。</p></div><div><button type="button" onClick={() => void confirmResultsToggle()} disabled={busy || Boolean(data.game?.voting_open)}>确认公布并永久冻结</button><button type="button" className="secondary" onClick={() => setPendingResultsVisible(null)} disabled={busy}>取消</button></div></section>}
