...
Migration `202608250018` adds an owner-confirmed commercial quote request between the project draft and any future order/payment integration. It stores an immutable, closed-shape snapshot of the template, plan, wedding date and location, capacity, theme, tone, modules, and delivery scope. It excludes story material, boundaries, host notes, guest records, runtime state, and credentials. Project collaborators can read requests but only the owner can submit one; staff see a separate pending-request queue. A change to any commercial field automatically supersedes the active request. Submitting a request never assigns a price, creates an order or contract, charges a customer, or activates an entitlement.
...
Migration `202608250022` adds an immutable, server-derived fulfillment plan after the approved manifest is locked. A narrowly configured template/self-service/self-check project enters `standard_auto / awaiting_payment`; any human-dependent service, guided or bespoke customization, assisted operation, full rehearsal, or free-form service note enters `custom_service / manual_setup`. The authenticated browser cannot choose the lane or insert a plan. The staff-only RPC derives it from the stored approved delivery scope, serializes concurrent attempts, supports idempotent retries, and records an audit event. Runtime registration now also requires an exact matching plan. Planning does not create an order, verify a payment, activate an entitlement, call a provider, or create a paid resource.
...
Once the manifest and matching fulfillment plan are locked and the entitlement is explicitly `active`, an operator can register an already-created isolated runtime target. The registry stores only the exact manifest version/hash, a public HTTPS origin and a non-secret deployment reference. It rejects credentials, URL paths, query strings and fragments; it never stores provider tokens or database connection strings. Registration does not create resources and does not make a server-side request to the target.
