...
type ScoreboardData = {
  ...
  leaders: Array<{ id: string; name: string; team: string; points: number; completedTasks: number; undetectedTrickster: boolean; caughtTrickster: boolean }>;
...
export default function ScoreboardPage() {
  ...
  return <main className="scoreboard-shell">
    ...
    {data.leaders.length > 0 && <section className="scoreboard-panel"><div className="scoreboard-title"><div><small>INDIVIDUAL HONORS</small><h2>{data.resultsVisible ? '最终个人排名' : '个人荣誉榜'}</h2></div><span>{data.resultsVisible ? `共 ${data.leaders.length} 人` : `TOP ${Math.min(10, data.leaders.length)}`}</span></div>{data.resultsVisible && data.leaders.some((guest) => guest.undetectedTrickster || guest.caughtTrickster) && <p className="final-ranking-note">恶作剧者的任务分与游戏分只用于婚礼中的身份伪装，不进入最终名次：成功逃脱者置顶，被识破者置底，两者均不显示积分；其他宾客按个人总分排序。</p>}<ol className="leaderboard-list">{data.leaders.map((guest, index) => { const tricksterResult = guest.undetectedTrickster ? '成功逃脱' : guest.caughtTrickster ? '已被识破' : null; return <li className={guest.undetectedTrickster ? 'undetected-trickster' : guest.caughtTrickster ? 'caught-trickster' : ''} key={guest.id}><b>{String(index + 1).padStart(2, '0')}</b><div><strong>{guest.name}{guest.undetectedTrickster && <em>完美伪装</em>}{guest.caughtTrickster && <em>已被识破</em>}</strong><small>{guest.team} · {tricksterResult ? '终局按身份结果结算' : `完成 ${guest.completedTasks} 项任务`}</small></div><span className={tricksterResult ? 'trickster-result' : ''}>{tricksterResult ?? guest.points}</span></li>; })}</ol></section>}
