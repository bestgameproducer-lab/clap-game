...
export default function AdminPage() {
  ...
  return <main className="admin-shell">
    ...
    <section className="section-card finale-workflow-card">
      ...
      {pendingResultsVisible !== null && <section className="finale-confirmation" role="dialog" aria-label="确认公布身份"><div><small>请确认终局操作</small><strong>{pendingResultsVisible ? '公布身份并结算全部积分' : '暂时隐藏公开身份'}</strong><p>{pendingResultsVisible ? `当前收到 ${data.votes.length} 票。继续后将自动关闭投票，并一次性结算投票、团队奖励和第二轮能力。` : '已经结算的个人和团队积分不会撤销；如需重新投票，必须开启新一轮。'}</p></div><div><button type="button" onClick={() => void confirmResultsToggle()} disabled={busy}>{pendingResultsVisible ? '确认公布并结算' : '确认隐藏'}</button><button type="button" className="secondary" onClick={() => setPendingResultsVisible(null)} disabled={busy}>取消</button></div></section>}
